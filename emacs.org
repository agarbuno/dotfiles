...
- [[#package-system-setup][Package System Setup]]
  ...
  - [[#no-litering][No Litering]]
  - [[#automatic-package-updates][Automatic Package Updates]]
  - [[#inmortal-buffers][Inmortal buffers]]
...
#+begin_src emacs-lisp
  ...
  (defun agarbuno/immortal-buffers ()
  ...
  (add-hook 'kill-buffer-query-functions 'agarbuno/immortal-buffers)
...
The =agarbuno/org-font-setup= function configures various text faces to tweak the sizes of headings and use variable width fonts in most cases so that it looks more like we're editing a document in =org-mode=.  We switch back to fixed width (monospace) fonts for code blocks and tables so that they display correctly.
...
This snippet adds a hook to =org-mode= buffers so that =agarbuno/org-babel-tangle-config= gets executed each time such a buffer gets saved.  This function checks to see if the file being saved is the Emacs.org file you're looking at right now, and if so, automatically exports the configuration here to the associated output files.
...
#+begin_src emacs-lisp
  (defun agarbuno/latex-mode-visual-fill ()
    (setq fill-column 80)
  ...
  (use-package latex
    :ensure auctex
    :hook (LaTeX-mode . agarbuno/latex-mode-visual-fill)
    :custom
...
#+begin_src emacs-lisp
  (use-package bibtex
    ...
    :config
    (defun agarbuno/bibtex-generate-autokey ()
      (interactive)
      ...
      (let* ((names (bibtex-autokey-get-names))
        ...
        (let ((ret key))
          ...
          ret)))
    (advice-add #'bibtex-generate-autokey :override #'agarbuno/bibtex-generate-autokey)
    )
...
An alternative to =org-tree-slide= seems to be easier to customize. 

#+begin_src emacs-lisp
  (defun agarbuno/org-present-prepare-slide ()
    (org-overview)
    (org-show-entry)
    (org-show-children))

  (defun agarbuno/org-present-hook ()
    ...
    (agarbuno/org-present-prepare-slide))

  (defun agarbuno/org-present-quit-hook ()
  ...
  (defun agarbuno/org-present-prev ()
    (interactive)
    (org-present-prev)
    (agarbuno/org-present-prepare-slide))

  (defun agarbuno/org-present-next ()
    (interactive)
    (org-present-next)
    (agarbuno/org-present-prepare-slide))
  ...
  (use-package org-present
    :bind (:map org-present-mode-keymap
           ("C->" . agarbuno/org-present-next)
           ("C-<" . agarbuno/org-present-prev))
    :hook ((org-present-mode . agarbuno/org-present-hook)
           (org-present-mode-quit . agarbuno/org-present-quit-hook)))
#+end_src
** Org-tree-slide
...
#+begin_src emacs-lisp
  (defun agarbuno/org-start-presentation ()
    (interactive)
  ...
  (defun agarbuno/org-end-presentation ()
    (interactive)
    (text-scale-mode 0)
    (org-sticky-header-mode 1)
  ...
  (use-package org-tree-slide
    ...
    (evil-define-key 'normal org-tree-slide-mode-map
      (kbd "q") 'agarbuno/org-end-presentation
